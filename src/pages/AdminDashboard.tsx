...
// import ModuleReportDownload from "../components/ModuleReportDownload.js"
...
const AdminDashboard = () => {
  ...
  const deleteSelectedUsers = async () => {
    ...
    try {
      ...
      fetchUsers(filter); // refresh after delete
  ...
  const toggleChip = (chipId: number) => {
  ...
  const handleParameterChange = (parameterId: number, newValue, field) => {
    setParameters((prev) =>
      prev.map((p) =>
  ...
  const handleModuleChange = (moduleId: number, newName: string) => {
    setModules((prev) =>
      prev.map((p) =>
  ...
  // Add new option
  // const handleAddOption = (parameterId: number) => {
  //   setParameters((prev) =>
  //     prev.map((p) =>
  //       p.id === parameterId
  //         ? {
  ...
  //           options: [
  //             ...p.options,
  //             { id: Date.now(), name: "", value: "" }, // temporary ID
  //           ],
  //         }
  //         : p
  //     )
  ...
  // Apply changes: call backend
  const handleApplyChanges = async (id: number) => {
    try {
  ...
  //   if (optId > 100000) return;
  //   if (!window.confirm("Are you sure you want to delete selected users?")) return;
  //   try {
  //     setIsLoadingParameter(true);
  //     let res = await axios.delete(`http://localhost:5000/api/users/option/${optId}`);
  //     console.log(res);
  //     alert("Selected option deleted successfully!");
  //   } catch (error) {
  //     console.log(error);
  //     alert("There is an error while deleting the option, Please try again.");
  //   } finally {
  ...
  //   reader.onload = (evt) => {
  //     const base64Image = evt.target.result; // Base64 string
  //     console.log("Base64 Image:", base64Image);

  //     // Example: Send image to backend
  //     // uploadImage(file.name, base64Image);
  //   };
            ...
            <div className="tabs-container">
              <div className="tab-container">
                ...
                <div className="tab" onClick={() => handleTabChange(2)}>
                  <div className={"indicator" + (selectedTab === 2 ? " active-indicator" : "")}>EDIT CONFIGURATION</div>
                </div>
              </div>
            </div>
          </div>
          <div className="admin-nav-option-container">
            <div style={{ color: "red", cursor: "pointer", display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center", fontSize: "19px", fontWeight: "600", gap: "7px" }} onClick={() => {
              localStorage.clear();
              navigate("/login");
            }}>LOGOUT <LogoutIcon style={{ fontSize: "18px" }} /></div>
          </div>
        </div>
      </nav>
            ...
            <Paper sx={{ height: 400, width: '100%' }}>
              <DataGrid
                ...
                onRowSelectionModelChange={(data) => {
                  const dataArray = Array.from(data.ids);
              ...
              <div className="search-container">
                ...
                <div className="search-results-container">
                  {searchedString && filteredUsers.length > 0 ? (
                    ...
                    <ul>
                      {users.map((user) => (
                        <li key={user.id} style={{listStyle: "none"}} onClick={() => handleUserListClick(user)} style={{ padding: "6px", borderBottom: "1px solid #ddd", fontSize: "17px" }}>
                          <strong>{user.firstname} {user.lastname}</strong> - {user.email} - {user.type === 1 ? "User" : user.type === 2 ? "Supervisor" : user.type === 3 ? "Manager" : "Admin"}
                        </li>
                      ))}
                    </ul>
